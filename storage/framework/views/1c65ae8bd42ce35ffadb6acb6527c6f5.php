<?php
$useCustomRegister = $customRegister ?? false;
?>
<div class="package-plans">
    <div class="container">
        <div class="row g-sm-4 g-3">
            <?php $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
            $price = number_format($plan->price, 2, '.', '');
            [$whole, $decimal] = explode('.', $price);
            $planName = strtolower($plan->name);
            switch ($planName) {
                case 'bronze':
                $buttonText = "Start with {$plan->name} <span class='d-none d-md-inline'>- \${$whole}</span>";
                break;
                case 'silver':
                $buttonText = "Choose {$plan->name} <span class='d-none d-md-inline'>- \${$whole}</span>";
                break;
                case 'gold':
                $buttonText = "Buy {$plan->name} <span class='d-none d-md-inline'>- \${$whole}</span>";
                break;
                case 'subscription':
                $buttonText = "Go {$plan->name} <span class='d-none d-md-inline'>- \${$whole}</span>";
                break;
                default:
                $buttonText = "Start with {$plan->name} <span class='d-none d-md-inline'>- \${$whole}</span>";
            }
            ?>
            <div class="col-xl-3 col-6 <?php echo e(strtolower($plan->name)); ?> position-relative">
                <img src="images/most-popular-img-2.png" class="popular-badge">
                <p class="mb-0 popular-badge-text">Most Popular</p>
                <div class="access-box <?php echo e(strtolower($plan->name)); ?>">
                    <div class="inner-wrapper">                            
                        <div>
                            <div class="access-title">
                                <p class="mb-0"><?php echo e($plan->name); ?></p>
                            </div>
                            <div class="package-desc mt-3">
                                <div class="plan-details py-2 px-md-3 px-2">
                                   <div class="plan-price">
                                      <sup>$</sup><?php echo e($whole); ?><sup>.<?php echo e($decimal); ?></sup>
                                      <h3 class="mob-title d-block d-md-none"><?php echo e($plan->price_description); ?></h3>
                                  </div>
                                  <div class="desktop-mobile-description">
                                      <div class="plan-description desktop_description d-none d-md-block">
                                       <?php echo $plan->description; ?>

                                   </div>
                                   <div class="plan-description mobile_description d-block d-md-none">
                                       <?php echo $plan->small_description; ?>

                                   </div>

                                    <p class="cancel-anytime">Cancel Anytime</p> 

                               </div>

                             </div>
                         </div>
                     </div>
                     <div class="w-100">
                      <?php if(!empty($plan->read_more)): ?>
                      <button type="button"
                      class="read-more-circle"
                      data-bs-toggle="modal"
                      data-bs-target="#planModal-<?php echo e($plan->id); ?>">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 23.625 23.625"><path fill="#030104" d="M11.812 0C5.289 0 0 5.289 0 11.812s5.289 11.813 11.812 11.813 11.813-5.29 11.813-11.813S18.335 0 11.812 0zM14.271 18.307c-.608.24-1.092.422-1.455.548-.362.126-.783.189-1.262.189-.736 0-1.309-.18-1.717-.539s-.611-.814-.611-1.367c0-.215.015-.435.045-.659.031-.224.08-.476.147-.759l.761-2.688c.067-.258.125-.503.171-.731.046-.23.068-.441.068-.633 0-.342-.071-.582-.212-.717-.143-.135-.412-.201-.813-.201-.196 0-.398.029-.605.09-.205.063-.383.12-.529.176l.201-.828c.498-.203.975-.377 1.43-.521.455-.146.885-.218 1.29-.218.731 0 1.295.178 1.692.53.395.353.594.812.594 1.376 0 .117-.014.323-.041.617-.027.295-.078.564-.152.811l-.757 2.68c-.062.215-.117.461-.167.736-.049.275-.073.485-.073.626 0 .356.079.599.239.728.158.129.435.194.827.194.185 0 .392-.033.626-.097.232-.064.4-.121.506-.17l.12.828zM14.137 7.429c-.353.328-.778.492-1.275.492-.496 0-.924-.164-1.28-.492-.354-.328-.533-.727-.533-1.193 0-.465.18-.865.533-1.196.356-.332.784-.497 1.28-.497.497 0 .923.165 1.275.497.353.331.53.731.53 1.196 0 .467-.177.865-.53 1.193z"/></svg>
                  </button>
                  <?php endif; ?>
                  <div class="access-btn">
                    <?php if(auth()->check()): ?>
                    <a href="<?php echo e(route('subscription.show', $plan->id)); ?>">
                        <?php echo $buttonText; ?>

                    </a>
                    <?php else: ?>
                    <a class="read-more-circle"
                    data-bs-toggle="modal"
                    data-bs-target="#subscriptionplanModal-<?php echo e($plan->id); ?>">
                       <?php echo $buttonText; ?>

                   </a>
                <?php endif; ?>
                </div>
            </div>
        </div>
    </div>
</div>
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
</div>
</div>
</div>

<?php /**PATH /home/1476201.cloudwaysapps.com/udhewsxxeh/public_html/resources/views/consumer/blocks/pop-up-subscription-package-list.blade.php ENDPATH**/ ?>