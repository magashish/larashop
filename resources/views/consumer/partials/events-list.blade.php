...
<div class="sec-tag mt-4 mb-3 rounded-pill d-inline-block month-header {{$event_category}}-{{ Str::slug($monthYear, '-') }}">
...
<div class="row g-2">
    ...
    <div class="col-xl-2 col-md-6 order-2 d-lg-block d-none">
        <div class="inner-bg sec-img d-flex align-items-end">
            <img
                src="{{ $event->image ? asset('storage/'.$event->image) : asset('images/australian-dollar.png') }}"
                class="img-fluid"
                alt="{{ $event->name }}">
        </div>
    </div>
    ...
    <div class="col-xl-7 order-xl-2 order-1">
        <div class="inner-bg content-sec p-lg-4 p-3">
            ...
            <h2 class="d-lg-inline-block d-none">{{ $event->name }}</h2>
            ...
            <p class="text-sm text-gray-700 mt-2">
                @if($event->start)
                    <strong>Start:</strong> {{ \Carbon\Carbon::parse($event->start)->format('M d, Y h:i A') }}
                @endif
                @if($event->end)
                    &nbsp; | &nbsp;<strong>End:</strong> {{ \Carbon\Carbon::parse($event->end)->format('M d, Y h:i A') }}
                @endif
            </p>
