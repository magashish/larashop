...
<section class="image-banner-sec pb-lg-4">
    <div class="container">
        <div class="inner-content">
         @php $pastWinnerJed = $metaData['mazda_hero_image'] ?? 'jed.png'; @endphp
         <img src="{{ Str::contains($pastWinnerJed, '/') ? Storage::url($pastWinnerJed) : asset('images/Mazda_Aspley_Banner.webp') }}">

          @php $mazda_sub_hero_image = $metaData['mazda_sub_hero_image'] ?? 'jed.png'; @endphp
            <img src="{{ Str::contains($mazda_sub_hero_image, '/') ? Storage::url($mazda_sub_hero_image) : asset('images/Mazda_Aspley_Banner.webp') }}" class="side-img d-lg-block d-none">
     </div>
     <div class="bg-title">
        <div class="yellow-bg">{{ $metaData['mazda_title'] ?? 'UNLOCK YOUR' }}</div>
        <div class="black-bg"><h1>{{ $metaData['mazda_subtitle'] ?? 'UNLOCK YOUR' }}</h1></div>
    </div>
</div>
</section>

@php
$offerIds = json_decode($metaData['mazda_featured_offers'] ?? '[]', true);
@endphp
@php
...
<section class="exclusive-offers-section">
    <div class="container">
        ...
        <h2 class="exclusive-title">
            {!! $metaData['mazda_exclusive_title'] ?? 'check their exclusive offers' !!}
        </h2>
    </div>
        ...
        <div class="exclusive-cta">
            <span>WANT THESE EXCLUSIVE OFFERS? SIGN UP TODAY</span>
                ...
                <div class="xhale-access-row">
                    ...
                    <div class="sec-head">
                        <div class="bg-title mx-0 mb-5">
                            <div class="yellow-bg">{{ $metaData['mazda_package_title'] ?? 'UNLOCK YOUR' }}</div>
                            <div class="black-bg">{{ $metaData['mazda_package_subtitle'] ?? 'XHALE ACCESS' }}</div>
                        </div>
                    </div>
                </div> 
            </div>          
...
<section class="past-winners">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-md-8 offset-md-4">
                <div class="bg-title small-title">
                    <div class="yellow-bg">{{ $metaData['mazda_past_winner_title'] ?? 'SOME OF OUR' }}</div>
                    <div class="black-bg">{{ $metaData['mazda_past_winner_subtitle'] ?? 'PAST WINNERS' }}</div>
                </div>

                <div class="past-winner-slider">
                    @php $pastWinners = json_decode($metaData['mazda_past_winner_slider'] ?? '[]', true); @endphp
        ...
        <div class="jed">
            @php $pastWinnerJed = $metaData['mazda_past_winner_image'] ?? 'jed.png'; @endphp
        ...
        <div class="row align-items-center">
            <div class="col-lg-6 col-12 order-lg-1 order-2">
                <div class="supporting-community-contnet mb-lg-0 mb-5">                  
                    <div class="supporting-community-inner">
                        <h2>
                            {{ $metaData['mazda_harcourt_title'] ?? 'Supporting Harcourt' }} 
                            <span>{{ $metaData['mazda_harcourt_subtitle'] ?? 'Community' }}</span>
                        </h2>

                        @php
                        $fullContent = $metaData['mazda_harcourt_description'] ?? '';
                        ...
                        {{-- @if(!empty($metaData['mazda_harcourt_description']))
                        {!! $metaData['mazda_harcourt_description'] !!}
                        @else
            ...
            <div class="col-lg-6 col-12 order-lg-2 order-1">
                <div class="supporting-img1">
                    @if(!empty($metaData['mazda_harcourt_image_1']) && Storage::disk('public')->exists($metaData['mazda_harcourt_image_1']))
                    <img src="{{ Storage::url($metaData['mazda_harcourt_image_1']) }}" alt="Harcourt Support 1">
                    @else
                    <img src="{{ asset('images/harcourt-img-1.jpg') }}" alt="Default Support 1">
                    @endif
                </div>
                <div class="supporting-img2">
                    @if(!empty($metaData['mazda_harcourt_image_2']) && Storage::disk('public')->exists($metaData['mazda_harcourt_image_2']))
                    <img src="{{ Storage::url($metaData['mazda_harcourt_image_2']) }}" alt="Harcourt Support 2">
                    @else
