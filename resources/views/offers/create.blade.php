	...
	<div class="row justify-content-center">
		<div class="col-md-12">
			<div class="card shadow">
				<div class="card-header fw-bold">{{ __('Offer / Create') }}</div>
				<div class="card-body">
					 @if (session('status'))
					...
					<form action="{{ route('offers.store') }}" method="POST" enctype="multipart/form-data">
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control datepicker @error('start_date') is-invalid @enderror" id="start_date" name="start_date" value="{{ old('start_date') }}" >
							@error('start_date')
							<div class="invalid-feedback d-block">{{ $message }}</div>
							@enderror
						</div>
						...
						<div class="mb-3">
							...
							<select class="form-select @error('processing_status') is-invalid @enderror" id="processing_status" name="processing_status" required>
								...
								<option value="{{ $value }}" {{ old('processing_status', \App\Enums\OrganisationStatus::NEW) == $value ? 'selected' : '' }}>
									{{ $label }}
								</option>
								@endforeach
							</select>
						...
						<button type="submit" class="btn btn-primary">
							<i class="bi bi-save"></i> Create Offer
						</button>
